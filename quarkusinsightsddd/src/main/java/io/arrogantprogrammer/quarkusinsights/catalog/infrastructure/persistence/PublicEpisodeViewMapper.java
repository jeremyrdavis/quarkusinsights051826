...
public class PublicEpisodeViewMapper {
    ...
    public PublicEpisodeView toView(PublicEpisodeViewEntity entity) {
        // AbstractText constructor validates the 100..5000-char range; a data-integrity
        // violation (should never happen with a correct projector) surfaces as
        // IllegalArgumentException rather than being silently swallowed.
        Optional<AbstractText> abstract_ = Optional.ofNullable(entity.abstractText)
            .map(AbstractText::new);
